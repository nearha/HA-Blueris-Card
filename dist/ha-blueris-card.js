(() => {
  ...
  const normalizeItems = (items, fallback) => {
    ...
    for (const item of items) {
      const id = String(item?.id ?? item?.optionValue ?? item ?? "").trim();
      const name = String(item?.name ?? item?.optionDisplay ?? item?.id ?? item ?? "").trim();
      if (!id) continue;
  ...
  class BlueIrisUi3Card extends HTMLElement {
    constructor() {
      ...
      this._onResize = () => this._scheduleAutoHeight();
    }
    ...
    async _load() {
      ...
      try {
        ...
        await this._makeUrl();
        this._notice = "Proxy autenticado pelo backend";
      } catch (error) {
        this._url = "";
    ...
    async _makeUrl() {
      ...
      const payload = await this._hass.callApi("GET", `${API}/${entryId}/ui3_url?${params}`);
      const url = payload.url || "";
      this._url = `${url}${url.includes("?") ? "&" : "?"}ha_card_nonce=${encodeURIComponent(`${Date.now()}_${this._nonce}`)}`;
    }
    ...
    _scheduleAutoHeight() {
      clearTimeout(this._heightTimer);
      this._heightTimer = setTimeout(() => this._applyAutoHeight(), 60);
    }

    _applyAutoHeight() {
      if (this._config.auto_height !== true) return;
      const frame = this.shadowRoot?.querySelector(".frame");
      if (!frame) return;

      const rect = frame.getBoundingClientRect();
      const margin = Number(this._config.auto_height_margin ?? 16);
      const available = Math.floor(window.innerHeight - rect.top - margin);
      const minHeight = 180;
      frame.style.height = `${Math.max(minHeight, available)}px`;
    }

    _frameHeightStyle() {
      if (this._config.auto_height === true) return "50vh";
      return this._config.height || "70vh";
    }

    _render() {
      const selectedGroup = this._groups.find((group) => group.id === this._group)?.name || this._group;
      const selectedProfile = this._profiles.find((profile) => profile.id === this._profile)?.name || this._profile;
      ...
      const subtitle = entry ? `Proxy autenticado · ${entry.title || "Blue Iris UI3"}` : "Proxy autenticado pelo backend";
        ...
        <ha-card class="bi-card">
          ...
          <div class="controls">
            <label>
              <span>Grupo</span>
              <select id="group">
                ${this._groups.map((group) => `<option value="${escapeHtml(group.id)}" ${group.id === this._group ? "selected" : ""}>${escapeHtml(group.name || group.id)}</option>`).join("")}
              </select>
            </label>
            <label>
              <span>Resolução</span>
              <select id="profile">
                ${this._profiles.map((profile) => `<option value="${escapeHtml(profile.id)}" ${profile.id === this._profile ? "selected" : ""}>${escapeHtml(profile.name || profile.id)}</option>`).join("")}
              </select>
            </label>
      ...
      this.shadowRoot.getElementById("group")?.addEventListener("change", (event) => this._setGroup(event.target.value));
      this.shadowRoot.getElementById("profile")?.addEventListener("change", (event) => this._setProfile(event.target.value));
      ...
      this.shadowRoot.getElementById("open")?.addEventListener("click", () => {
        if (this._url) window.open(this._url, "_blank", "noopener,noreferrer");
      });
  ...
  class BlueIrisUi3CardEditor extends HTMLElement {
    ...
    async _loadLists(force = false) {
      ...
      if (!this._config.default_group || !this._groups.some((group) => group.id === this._config.default_group)) {
        this._config.default_group = this._groups[0]?.id || "index";
      }

      if (!this._config.default_profile || !this._profiles.some((profile) => profile.id === this._config.default_profile)) {
        this._config.default_profile =
    ...
    _render() {
      ...
      this.shadowRoot.innerHTML = `
        ...
        <div class="editor">
          <section>
            <h3>Backend</h3>
            <label>Instância Blue Iris
              <select id="entry_id">
                <option value="">${this._entries.length ? "Selecionar" : "Nenhuma encontrada"}</option>
                ${this._entries.map((entry) => `<option value="${escapeHtml(entry.entry_id)}" ${entry.entry_id === c.entry_id ? "selected" : ""}>${escapeHtml(entry.title || "Blue Iris UI3")}</option>`).join("")}
              </select>
            </label>
            ${this._error ? `<p class="err">${escapeHtml(this._error)}</p>` : ""}
            <p>O backend autentica no Blue Iris e entrega a UI3 por proxy. A senha não fica no card.</p>
          </section>

          <section>
            <h3>Card</h3>
            <div class="grid">
              <label>Título<input id="title" value="${escapeHtml(c.title)}"></label>
              <label>Altura manual<input id="height" value="${escapeHtml(c.height)}"></label>
              <label>Grupo padrão<select id="default_group">${this._groups.map((group) => `<option value="${escapeHtml(group.id)}" ${group.id === c.default_group ? "selected" : ""}>${escapeHtml(group.name || group.id)}</option>`).join("")}</select></label>
              <label>Resolução padrão<select id="default_profile">${this._profiles.map((profile) => `<option value="${escapeHtml(profile.id)}" ${profile.id === c.default_profile ? "selected" : ""}>${escapeHtml(profile.name || profile.id)}</option>`).join("")}</select></label>
              <label>Timeout UI3<input id="timeout" type="number" value="${escapeHtml(c.timeout)}"></label>
              <label>Delay ao voltar<input id="resume_reload_delay" type="number" value="${escapeHtml(c.resume_reload_delay ?? 1200)}"></label>
              <label>Margem inferior auto<input id="auto_height_margin" type="number" value="${escapeHtml(c.auto_height_margin ?? 16)}"></label>
            </div>
            <div class="checks">
              <label><input id="auto_height" type="checkbox" ${c.auto_height === true ? "checked" : ""}>Altura automática</label>
              <label><input id="maximize" type="checkbox" ${c.maximize !== false ? "checked" : ""}>UI3 maximizada</label>
      ...
      const bindValue = (id, key) => this.shadowRoot.getElementById(id)?.addEventListener("change", (event) => this._setConfigValue(key, event.target.value));
  ...
  if (!window.customCards.some((card) => card.type === "blueiris-ui3-card")) {
    window.customCards.push({
      ...
      name: "Blue Iris UI3",
      description: "Blue Iris UI3 proxy autenticado pelo backend",
      preview: false,
    });
  }

  console.info(
    `%c HA-BLUERIS-CARD %c v${VERSION} authenticated-proxy loaded `,
