...
(() => {
  ...
  class BlueIrisUi3Card extends HTMLElement {
    ...
    async _buildUrl() {
      ...
      if ((auth === "session" || auth === "auto") && (this._config.username || this._config.password)) {
        try {
          const session = await this._ensureSession();
          if (session) url.searchParams.set("session", session);
        } catch (err) {
          if (auth === "session") this._notice = `Login de sessão falhou: ${msg(err)}`;
          if (auth === "auto" && this._config.username && this._config.password) {
            url.searchParams.set("user", this._config.username);
            url.searchParams.set("pw", this._config.password);
            this._notice = `Sessão falhou; usando user/pw na URL (${msg(err)})`;
          }
        }
      } else if (auth === "url" && this._config.username && this._config.password) {
        url.searchParams.set("user", this._config.username);
        url.searchParams.set("pw", this._config.password);
      }
    ...
    async _camlist() {
      if ((this._config.direct_auth === "session" || this._config.direct_auth === "auto") && (this._config.username || this._config.password)) await this._ensureSession();
      return this._json("camlist", {});
    }
    async _ensureSession() {
      ...
      const first = await this._json("login", {}, true);
      const session = first.session || first.data?.session || "";
      if (!session) throw new Error("Blue Iris não retornou session");
      const response = md5(`${this._config.username}:${session}:${this._config.password}`);
      const auth = await this._json("login", { session, response }, true);
      if (auth.result !== "success") throw new Error(auth.reason || auth.data || "login falhou");
      this._session = auth.session || auth.data?.session || session;
      this._sessionAt = Date.now();
      return this._session;
    }
    async _json(cmd, payload = {}, noSession = false) {
      const url = new URL(`json?_${encodeURIComponent(cmd)}`, this._base());
      const body = { cmd, ...payload };
      if (!noSession && this._session) body.session = this._session;
  ...
  class BlueIrisUi3CardEditor extends HTMLElement {
    ...
    _render() {
      ...
      this.shadowRoot.innerHTML = `<style>${editorStyles()}</style><div class="ed">
        <section><h3>Blue Iris</h3><div class="grid3"><label>Host/IP<input id="host" value="${ea(c.host)}"></label><label>Porta<input id="port" type="number" value="${ea(c.port)}"></label><label class="chk"><input id="ssl" type="checkbox" ${c.ssl ? "checked" : ""}> HTTPS</label></div><div class="grid2"><label>Usuário<input id="username" value="${ea(c.username)}" autocomplete="off"></label><label>Senha<input id="password" type="password" value="${ea(c.password)}" autocomplete="new-password"></label></div><label>Autenticação<select id="direct_auth"><option value="none" ${c.direct_auth === "none" ? "selected" : ""}>Sem login / liberado na LAN</option><option value="session" ${c.direct_auth === "session" ? "selected" : ""}>Sessão via API</option><option value="url" ${c.direct_auth === "url" ? "selected" : ""}>user/pw na URL</option><option value="auto" ${c.direct_auth === "auto" ? "selected" : ""}>Auto</option></select></label></section>
        ...
        <section><h3>Listas</h3><div class="grid2"><label>Grupos manuais<textarea id="manual_groups">${esc(formatList(c.manual_groups))}</textarea></label><label>Perfis<textarea id="profiles">${esc(formatList(c.profiles))}</textarea></label></div><p>Formato: <code>id|Nome</code>, um por linha.</p></section>
      </div>`;
  ...
  function normalizeConfig(c) { c.port = Number(c.port || 80); c.timeout = Number(c.timeout || 0); c.direct_auth = ["none", "session", "url", "auto"].includes(c.direct_auth) ? c.direct_auth : "none"; c.manual_groups = normalizeItems(c.manual_groups, DEFAULT_GROUPS); c.profiles = normalizeItems(c.profiles, DEFAULT_PROFILES); return c; }
  ...
  function md5(str){function cmn(q,a,b,x,s,t){a=add32(add32(a,q),add32(x,t));return add32((a<<s)|(a>>>(32-s)),b)}function ff(a,b,c,d,x,s,t){return cmn((b&c)|((~b)&d),a,b,x,s,t)}function gg(a,b,c,d,x,s,t){return cmn((b&d)|(c&(~d)),a,b,x,s,t)}function hh(a,b,c,d,x,s,t){return cmn(b^c^d,a,b,x,s,t)}function ii(a,b,c,d,x,s,t){return cmn(c^(b|(~d)),a,b,x,s,t)}function cycle(x,k){let[a,b,c,d]=x;a=ff(a,b,c,d,k[0],7,-680876936);d=ff(d,a,b,c,k[1],12,-389564586);c=ff(c,d,a,b,k[2],17,606105819);b=ff(b,c,d,a,k[3],22,-1044525330);a=ff(a,b,c,d,k[4],7,-176418897);d=ff(d,a,b,c,k[5],12,1200080426);c=ff(c,d,a,b,k[6],17,-1473231341);b=ff(b,c,d,a,k[7],22,-45705983);a=ff(a,b,c,d,k[8],7,1770035416);d=ff(d,a,b,c,k[9],12,-1958414417);c=ff(c,d,a,b,k[10],17,-42063);b=ff(b,c,d,a,k[11],22,-1990404162);a=ff(a,b,c,d,k[12],7,1804603682);d=ff(d,a,b,c,k[13],12,-40341101);c=ff(c,d,a,b,k[14],17,-1502002290);b=ff(b,c,d,a,k[15],22,1236535329);a=gg(a,b,c,d,k[1],5,-165796510);d=gg(d,a,b,c,k[6],9,-1069501632);c=gg(c,d,a,b,k[11],14,643717713);b=gg(b,c,d,a,k[0],20,-373897302);a=gg(a,b,c,d,k[5],5,-701558691);d=gg(d,a,b,c,k[10],9,38016083);c=gg(c,d,a,b,k[15],14,-660478335);b=gg(b,c,d,a,k[4],20,-405537848);a=gg(a,b,c,d,k[9],5,568446438);d=gg(d,a,b,c,k[14],9,-1019803690);c=gg(c,d,a,b,k[3],14,-187363961);b=gg(b,c,d,a,k[8],20,1163531501);a=gg(a,b,c,d,k[13],5,-1444681467);d=gg(d,a,b,c,k[2],9,-51403784);c=gg(c,d,a,b,k[7],14,1735328473);b=gg(b,c,d,a,k[12],20,-1926607734);a=hh(a,b,c,d,k[5],4,-378558);d=hh(d,a,b,c,k[8],11,-2022574463);c=hh(c,d,a,b,k[11],16,1839030562);b=hh(b,c,d,a,k[14],23,-35309556);a=hh(a,b,c,d,k[1],4,-1530992060);d=hh(d,a,b,c,k[4],11,1272893353);c=hh(c,d,a,b,k[7],16,-155497632);b=hh(b,c,d,a,k[10],23,-1094730640);a=hh(a,b,c,d,k[13],4,681279174);d=hh(d,a,b,c,k[0],11,-358537222);c=hh(c,d,a,b,k[3],16,-722521979);b=hh(b,c,d,a,k[6],23,76029189);a=hh(a,b,c,d,k[9],4,-640364487);d=hh(d,a,b,c,k[12],11,-421815835);c=hh(c,d,a,b,k[15],16,530742520);b=hh(b,c,d,a,k[2],23,-995338651);a=ii(a,b,c,d,k[0],6,-198630844);d=ii(d,a,b,c,k[7],10,1126891415);c=ii(c,d,a,b,k[14],15,-1416354905);b=ii(b,c,d,a,k[5],21,-57434055);a=ii(a,b,c,d,k[12],6,1700485571);d=ii(d,a,b,c,k[3],10,-1894986606);c=ii(c,d,a,b,k[10],15,-1051523);b=ii(b,c,d,a,k[1],21,-2054922799);a=ii(a,b,c,d,k[8],6,1873313359);d=ii(d,a,b,c,k[15],10,-30611744);c=ii(c,d,a,b,k[6],15,-1560198380);b=ii(b,c,d,a,k[13],21,1309151649);a=ii(a,b,c,d,k[4],6,-145523070);d=ii(d,a,b,c,k[11],10,-1120210379);c=ii(c,d,a,b,k[2],15,718787259);b=ii(b,c,d,a,k[9],21,-343485551);x[0]=add32(a,x[0]);x[1]=add32(b,x[1]);x[2]=add32(c,x[2]);x[3]=add32(d,x[3])}function blk(s){let a=[];for(let i=0;i<64;i+=4)a[i>>2]=s.charCodeAt(i)+(s.charCodeAt(i+1)<<8)+(s.charCodeAt(i+2)<<16)+(s.charCodeAt(i+3)<<24);return a}function md51(s){let n=s.length,state=[1732584193,-271733879,-1732584194,271733878],i;for(i=64;i<=n;i+=64)cycle(state,blk(s.substring(i-64,i)));s=s.substring(i-64);let tail=Array(16).fill(0);for(i=0;i<s.length;i++)tail[i>>2]|=s.charCodeAt(i)<<((i%4)<<3);tail[i>>2]|=0x80<<((i%4)<<3);if(i>55){cycle(state,tail);tail.fill(0)}tail[14]=n*8;cycle(state,tail);return state}function rhex(n){let s="";for(let j=0;j<4;j++)s+=((n>>(j*8+4))&15).toString(16)+((n>>(j*8))&15).toString(16);return s}function add32(a,b){return(a+b)&0xffffffff}return md51(unescape(encodeURIComponent(str))).map(rhex).join("")}
