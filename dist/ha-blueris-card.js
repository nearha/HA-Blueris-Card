...
(() => {
  ...
  class BlueIrisUi3Card extends HTMLElement {
    ...
    async _loadAll() {
      ...
      try {
        ...
        this._groups = items(gp.groups, DEFAULT_GROUPS); this._profiles = items(pp.profiles, DEFAULT_PROFILES); this._ensure(); await this._refreshUrl(); this._notice = `Backend conectado · ${this._groups.length} grupos`;
      } catch (err) { this._error = msg(err); }
    ...
    async _setGroup(v) { this._group = v; try { await this._refreshUrl(); this._error = ""; } catch (err) { this._error = msg(err); } this._render(); }
    async _setProfile(v) { this._profile = v; try { await this._refreshUrl(); this._error = ""; } catch (err) { this._error = msg(err); } this._render(); }
    _render() {
      ...
      const groupName = this._groups.find(g => g.id === this._group)?.name || this._group; const profileName = this._profiles.find(p => p.id === this._profile)?.name || this._profile;
      this.shadowRoot.innerHTML = `<style>${styles()}</style><ha-card class="bi-card">${this._config.show_header !== false ? `<div class="top"><div><div class="title">${h(this._config.title || "Blue Iris UI3")}</div><div class="sub">${h(entry ? `Backend · ${entry.host}:${entry.port}` : "Backend Home Assistant")}</div></div><span class="pill" title="${a(status)}">${h(status)}</span></div>` : ""}<div class="controls"><label><span>Grupo</span><select id="group">${this._groups.map(g => `<option value="${a(g.id)}" ${g.id === this._group ? "selected" : ""}>${h(g.name || g.id)}</option>`).join("")}</select></label><label><span>Resolução</span><select id="profile">${this._profiles.map(p => `<option value="${a(p.id)}" ${p.id === this._profile ? "selected" : ""}>${h(p.name || p.id)}</option>`).join("")}</select></label><button id="refresh">Atualizar</button>${this._config.show_open_button !== false ? `<button id="open">Abrir</button>` : ""}</div><div class="frame" style="height:${a(this._config.height || "70vh")}">${this._url ? `<iframe src="${a(this._url)}" allow="fullscreen; autoplay" referrerpolicy="same-origin"></iframe>` : ""}${this._error || (!this._url && this._loading) ? `<div class="overlay"><b>${this._error ? "Não carregou" : "Preparando UI3"}</b><span>${h(this._error || "Chamando API do backend...")}</span></div>` : ""}</div>${this._config.show_footer !== false ? `<div class="foot"><span>${h(groupName)} · ${h(profileName)}</span><span>v${VERSION}</span></div>` : ""}</ha-card>`;
      this.shadowRoot.getElementById("group")?.addEventListener("change", e => this._setGroup(e.target.value)); this.shadowRoot.getElementById("profile")?.addEventListener("change", e => this._setProfile(e.target.value)); this.shadowRoot.getElementById("refresh")?.addEventListener("click", () => this._loadAll()); this.shadowRoot.getElementById("open")?.addEventListener("click", () => this._url && window.open(this._url, "_blank", "noopener,noreferrer"));
    }
  }
  ...
  class BlueIrisUi3CardEditor extends HTMLElement {
    ...
    async _loadEntries() { if (!this._hass) return; try { const p = await this._hass.callApi("GET", `${API}/entries`); this._entries = p.entries || []; this._error = ""; if (!this._config.entry_id && this._entries.length === 1) { this._config.entry_id = this._entries[0].entry_id; this._emit(); } } catch (err) { this._entries = []; this._error = "Instale e configure a integração Blue Iris UI3 Backend primeiro."; } this._render(); }
    _set(k, v) { const n = { ...this._config, [k]: v }; if (k === "timeout") n[k] = Number(v || 0); if (["maximize", "show_header", "show_footer", "show_open_button"].includes(k)) n[k] = !!v; this._config = n; this._emit(); this._render(); }
    _emit() { this.dispatchEvent(new CustomEvent("config-changed", { detail: { config: this._config }, bubbles: true, composed: true })); }
    _render() { const c = this._config; this.shadowRoot.innerHTML = `<style>${editorStyles()}</style><div class="ed"><section><h3>Backend</h3><label>Instância Blue Iris<select id="entry_id"><option value="">${this._entries.length ? "Selecionar" : "Nenhuma encontrada"}</option>${this._entries.map(e => `<option value="${a(e.entry_id)}" ${e.entry_id === c.entry_id ? "selected" : ""}>${h(e.title || `${e.host}:${e.port}`)}</option>`).join("")}</select></label>${this._error ? `<p class="err">${h(this._error)}</p>` : ""}<p>O card não guarda senha e não autentica direto. Ele usa somente a API/proxy da integração.</p></section><section><h3>Card</h3><div class="grid2"><label>Título<input id="title" value="${a(c.title)}"></label><label>Altura<input id="height" value="${a(c.height)}"></label><label>Grupo padrão<input id="default_group" value="${a(c.default_group)}"></label><label>Perfil padrão<input id="default_profile" value="${a(c.default_profile)}"></label><label>Timeout UI3<input id="timeout" type="number" value="${a(c.timeout)}"></label></div><div class="checks"><label><input id="maximize" type="checkbox" ${c.maximize !== false ? "checked" : ""}> UI3 maximizada</label><label><input id="show_header" type="checkbox" ${c.show_header !== false ? "checked" : ""}> Cabeçalho</label><label><input id="show_footer" type="checkbox" ${c.show_footer !== false ? "checked" : ""}> Rodapé</label><label><input id="show_open_button" type="checkbox" ${c.show_open_button !== false ? "checked" : ""}> Botão abrir</label></div></section></div>`; const bind = (id,k,prop="value") => this.shadowRoot.getElementById(id)?.addEventListener("change", e => this._set(k, e.target[prop])); ["entry_id","title","height","default_group","default_profile","timeout"].forEach(id => bind(id,id)); ["maximize","show_header","show_footer","show_open_button"].forEach(id => bind(id,id,"checked")); }
  }

  function items(v, fallback) { if (!Array.isArray(v)) return [...fallback]; const out = v.map(x => ({ id: String(x?.id || x?.optionValue || "").trim(), name: String(x?.name || x?.optionDisplay || x?.id || "").trim() })).filter(x => x.id); return out.length ? out : [...fallback]; }
  function msg(e) { return e?.message || String(e || "Erro desconhecido"); }
  function h(v) { return String(v ?? "").replace(/[&<>'\"]/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'\"':"&quot;"}[c])); }
  function a(v) { return h(v); }
